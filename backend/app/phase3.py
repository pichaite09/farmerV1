...
def list_transactions(limit:int=Query(50),offset:int=Query(0),cycle_id:uuid.UUID|None=None,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def create_transaction(b:TransactionCreate,identity=Depends(farmer_session),db:Session=Depends(get_db)):
    u=user(identity)
...
def get_transaction(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):return tx_out(owned(db,Transaction,user(identity),i))
@router.patch('/transactions/{i}',response_model=TransactionOut)
def update_transaction(i:uuid.UUID,b:TransactionPatch,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def delete_transaction(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):
    t=owned(db,Transaction,user(identity),i);db.delete(t);db.commit();return Response(status_code=204)

@router.get('/vehicles',response_model=list[VehicleOut])
def list_vehicles(limit:int=Query(50),offset:int=Query(0),identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def create_vehicle(b:VehicleCreate,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def get_vehicle(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):return vehicle_out(owned(db,Vehicle,user(identity),i))
@router.patch('/vehicles/{i}',response_model=VehicleOut)
def patch_vehicle(i:uuid.UUID,b:VehiclePatch,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def delete_vehicle(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):
    x=owned(db,Vehicle,user(identity),i)
...
def list_fuel(limit:int=Query(50),offset:int=Query(0),vehicle_id:uuid.UUID|None=None,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def create_fuel(b:FuelCreate,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def get_fuel(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):return fuel_out(owned(db,FuelRecord,user(identity),i))
@router.patch('/fuel-records/{i}',response_model=FuelOut)
def patch_fuel(i:uuid.UUID,b:FuelPatch,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def delete_fuel(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def list_tasks(limit:int=Query(50),offset:int=Query(0),cycle_id:uuid.UUID|None=Query(None, alias='cycleId'),identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def create_task(b:TaskCreate,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def get_task(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):return task_out(db, owned(db,Task,user(identity),i))
@router.patch('/tasks/{i}',response_model=TaskOut)
def patch_task(i:uuid.UUID,b:TaskPatch,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def delete_task(i:uuid.UUID,identity=Depends(farmer_session),db:Session=Depends(get_db)):
...
def get_categories(identity=Depends(farmer_session),db:Session=Depends(get_db)):return categories(db,user(identity))
@router.put('/settings/categories/{key}',response_model=CategoriesOut)
def put_category(key:str,b:CategoryUpdate,if_match:str|None=Header(None),identity=Depends(farmer_session),db:Session=Depends(get_db)):
