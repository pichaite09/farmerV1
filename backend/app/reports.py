...
def dashboard(frm:date|None=Query(None,alias='from'),to:date|None=Query(None),identity=Depends(current_session),db:Session=Depends(get_db)):
    ...
    fuel_spend=sum((x.amount for x in fuel_rows),Decimal('0'))
    expense += fuel_spend
    bycat={}
    for x in tx:
